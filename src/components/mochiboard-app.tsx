...
const parseTimeToHour = (timeLabel: string | null) => {
  if (!timeLabel) {
    return null;
  }
...
const PlannerAccent = ({ theme }: { theme: ThemeKey }) => {
  const palette = themePalettes[theme];

  switch (theme) {
    case 'strawberry':
      return (
        <div className="pointer-events-none absolute right-3 top-3 flex gap-2 opacity-70" aria-hidden="true">
          <span className="inline-flex h-5 w-5 items-center justify-center rounded-full border border-[color:var(--border)]" style={{ backgroundColor: palette.accentSoft }}>
            <span className="text-[11px] text-[color:var(--accent-strong)]">🍓</span>
          </span>
          <span className="inline-flex h-5 w-5 items-center justify-center rounded-full border border-[color:var(--border)]" style={{ backgroundColor: palette.surface }}>
            <span className="text-[11px] text-[color:var(--accent-strong)]">❤</span>
          </span>
        </div>
      );
    case 'matcha':
      return (
        <div className="pointer-events-none absolute bottom-3 right-3 opacity-70" aria-hidden="true">
          <div className="flex items-end gap-1">
            <span className="text-lg text-[color:var(--accent-strong)]">🍵</span>
    ...
    case 'ube':
      return (
        <div className="pointer-events-none absolute left-3 top-3 opacity-70" aria-hidden="true">
    ...
    case 'black-sesame':
      return (
        <div className="pointer-events-none absolute bottom-3 left-3 opacity-70" aria-hidden="true">
          <div className="flex items-center gap-1">
            ...
            <span className="text-sm text-[color:var(--accent-strong)]">☾</span>
          </div>
        </div>
      );
    default:
...
export default function MochiboardApp({ activeView = 'dashboard' }: { activeView?: ViewMode }) {
  ...
  const [weekDays] = useState<PlannerDay[]>(() => getCurrentWeekDays());
  ...
  const scheduleSlots = [7, 9, 12, 15, 18, 21];
  const scheduleEntries = scheduleSlots.map((slot) => ({
    slotLabel: `${slot}:00`,
    tasks: visibleTasks.filter((task) => {
      const hour = parseTimeToHour(task.time_label);
      if (hour === null) {
        return false;
      }
      return Math.abs(hour - slot) <= 1;
    }),
  }));
          ...
          <main className="space-y-6">
            ...
            >
              ...
              <div className="relative z-10 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
                ...
                <div className="rounded-full bg-[color:var(--surface-soft)] px-4 py-2 text-sm font-medium text-[color:var(--accent-strong)] shadow-[var(--shadow-soft)]">
                  {selectedDayMeta ? `${selectedDayMeta.label} ${selectedDayMeta.dateNumber}` : 'Today'}
                </div>
              </div>
            </section>
              ...
              {weekDays.map((day) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    onClick={() => setSelectedDay(day.key)}
                    className={`min-w-[78px] rounded-[22px] border px-3 py-2 text-center text-sm transition ${
                      isActive
                        ? 'border-[color:var(--accent)] bg-[color:var(--accent-soft)] text-[color:var(--accent-strong)]'
                        : 'border-[color:var(--border)] bg-[color:var(--surface-soft)] text-[color:var(--muted)]'
                    }`}
            ...
            <section className="grid gap-6 xl:grid-cols-[1.1fr_1.3fr_0.9fr]">
              <div className="rounded-[30px] border border-[color:var(--border)] bg-[color:var(--surface)] p-5 shadow-[var(--shadow-soft)]">
                <div className="flex items-center justify-between">
                  <h3 className="text-xl font-semibold text-[color:var(--foreground)]">Schedule</h3>
                  <span className="rounded-full bg-[color:var(--accent-soft)] px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.24em] text-[color:var(--accent-strong)]">
                    Time blocks
                  </span>
                </div>
                <div className="mt-5 space-y-3">
                  {scheduleEntries.map((entry) => (
                    <div key={entry.slotLabel} className="rounded-[20px] border border-[color:var(--border)] bg-[color:var(--surface-soft)] p-3">
                      <div className="flex items-center justify-between text-sm font-semibold text-[color:var(--foreground)]">
                        <span>{entry.slotLabel}</span>
                        <span className="text-[color:var(--muted)]">{entry.tasks.length > 0 ? `${entry.tasks.length} task${entry.tasks.length > 1 ? 's' : ''}` : 'Open'}</span>
                      </div>
                      {entry.tasks.length > 0 ? (
                        <div className="mt-2 space-y-2">
                          {entry.tasks.map((task) => (
                            <div key={task.id} className="rounded-full border border-[color:var(--border)] bg-[color:var(--surface)] px-3 py-2 text-sm text-[color:var(--muted)]">
                              {task.title}
                            </div>
                          ))}
                        </div>
              ...
              <div className="rounded-[30px] border border-[color:var(--border)] bg-[color:var(--surface)] p-5 shadow-[var(--shadow-soft)]">
                ...
                <div className="mt-5 space-y-3">
                  {isLoadingTasks ? (
                    ...
                    visibleTasks.map((task) => (
                      <label key={task.id} className="flex items-start gap-3 rounded-[20px] border border-[color:var(--border)] bg-[color:var(--surface-soft)] p-3">
                        <input
                          type="checkbox"
                          checked={task.is_complete}
                          onChange={() => void handleToggleTask(task)}
              ...
              <div className="rounded-[30px] border border-[color:var(--border)] bg-[color:var(--surface)] p-5 shadow-[var(--shadow-soft)]">
                ...
                <div className="mt-5 rounded-[24px] border border-[color:var(--border)] bg-[color:var(--surface-soft)] p-4 shadow-[var(--shadow-soft)]" style={{ transform: 'rotate(-1deg)' }}>
                  <textarea
                    value={plannerNotes}
